...
static __always_inline int prepare_transmit_rawip(struct xdp_md *ctx)
{
	...
	struct ipv4_hdr *iph;
	u8 version;
	int ether_type_offset;
	int vlan_packet = 0;
	u16 ether_type = 0U;
#ifdef DEBUG
	const char module[] = "TX_RAWIP";
#endif
	...
	if ((void *)(iph + 1) > data_end) {
		bpf_debug("%s: Invalid iph(%p) + 1 > data_end(%p)\n",
				module, (void *)iph, data_end);
		goto pass;
	}
	...
	if (route->redir_if_type == ARPHRD_ETHER) {
		/* Redirect to ethernet interface, add ethernet header to IP packet */
		/* CMM doesn't count 2 bytes for ether_type in l2_hdr_size */
		if (bpf_xdp_adjust_head(ctx, 0 -(int)route->l2_hdr_size - 2)) {
			bpf_debug("%s: bpf_xdp_adjust_head(%d) failure => XDP_PASS\n",
					module, 0 -(int)route->l2_hdr_size - 2);
			goto pass;
		}
		...
		/* Copy VLAN info in Ethernet header if present */
		if (route->l2_hdr_size > (u16)(2 * ETHER_ADDR_LEN)) {
			ether_type_offset = 2 * ETHER_ADDR_LEN + (int)sizeof(struct vlan_hdr);
			vlan_packet = 1;
		...
		if (vlan_packet) {
			...
			/* Non VLAN packet. Set ether_type IPV4 or IPV6 */
			ether_type_offset = 2 * ETHER_ADDR_LEN;
		}

		if (version == IPV4_VERSION) {
			ether_type = htons(ETHERTYPE_IPV4);
		}
		else if (version == IPV6_VERSION) {
			ether_type = htons(ETHERTYPE_IPV6);
		}
		else {
			bpf_debug("%s: Unmanaged IP protocol(%u)\n", module, (unsigned int)version);
			goto pass;
		}

		__builtin_memcpy(data + ether_type_offset, &ether_type, sizeof(u16));
	}
	else if (route->redir_if_type == ARPHRD_RAWIP) {
		/* Redirect the IP packet to RAWIP interface */
	}
	else {
		bpf_debug("%s: Invalid redir_if_type(%u) => XDP_PASS\n",
				module, (unsigned int)(route->redir_if_type));
		goto pass;
	}

	bpf_debug("TX_RAWIP: Redirect packet:%llu (%llu) to interface:%d\n",
			stats->packets_fp[GLOB_RAWIP_TYPE], (u64)ctx->data_end - (u64)ctx->data, route->redir_ifindex);

	if (stats) {
		stats->packets_fp[GLOB_RAWIP_TYPE]++;
		stats->bytes_fp[GLOB_RAWIP_TYPE] += (u64)ctx->data_end - (u64)ctx->data;
	}
...
pass:
	bpf_debug("%s: => XDP_PASS(%llu)\n", module, (u64)ctx->data_end - (u64)ctx->data);
	if (stats) {
		stats->packets_sp[GLOB_RAWIP_TYPE]++;
		stats->bytes_sp[GLOB_RAWIP_TYPE] += (u64)ctx->data_end - (u64)ctx->data;
	}
...
static __always_inline int parse_ipv6_rawip(struct xdp_md *ctx)
{
	...
	void *data_end = (void *)(long)ctx->data_end;
	void *data = (void *)(long)ctx->data;
	struct ipv6hdr *iph = (struct ipv6hdr *)(data);
	int *ifindex, index_key_stat = GLOB_STAT;
	struct stats_entry *stats;
#ifdef DEBUG
	const char module[] = "IPV6_RAWIP";
#endif
...
#pragma clang loop unroll(full)
	...
	switch (protocol) {
		case IPPROTOCOL_UDP:
			...
			/* Check for IP datagram length validity */
			if ((htons(iph->payload_len) + IPV6_HEADER_LENGTH > info->mtu) ||
					((u16)((u64)data_end - (u64)data) > info->mtu)) {
				bpf_debug("%s/UDP: Invalid IP size (%u) > MTU (%u) => XDP_PASS\n",
						module, (unsigned int)(htons(iph->payload_len) + IPV6_HEADER_LENGTH), (unsigned int)(info->mtu));
				goto pass;
			}
		...
		case IPPROTOCOL_TCP:
			 ...
			 * value */
			if (tcph->rst || tcph->fin || tcph->syn) {
				bpf_debug("IPV6_RAWIP/TCP: rst(%u)/fin(%u)/syn(%u) => XDP_PASS\n",
						(unsigned int)(tcph->rst), (unsigned int)(tcph->fin), (unsigned int)(tcph->syn));
				goto pass;
			}
...
static __always_inline int parse_ipv4_rawip(struct xdp_md *ctx)
{
	...
	if (iph->ihl < 5U
			...
			|| iph->frag_offset & htons(IP_OFFSET | IP_MF)
			|| iph->ttl <= 1U) {
		bpf_debug("IPV4_RAWIP: Invalid IP header (%u %u %u) => XDP_PASS\n",
				(unsigned int)(iph->ihl), (unsigned int)(iph->version), (unsigned int)(iph->ttl));
		goto pass;
	}
	...
	switch (iph->protocol) {
		...
		case IPPROTOCOL_TCP:
			...
			if ((void *)(tcph + 1) > data_end) {
				bpf_debug("%s/TCP: Invalid tcph(%d) + 1 > data_end(%p) => XDP_PASS\n",
						module, (void *)tcph, data_end);
				return XDP_PASS;
			}
			 ...
			 * value */
			if (tcph->rst || tcph->fin || tcph->syn) {
				bpf_debug("IPV4_RAWIP/TCP: rst(%u)/fin(%u)/syn(%u) => XDP_PASS\n",
						(unsigned int)(tcph->rst), (unsigned int)(tcph->fin), (unsigned int)(tcph->syn));
				goto pass;
			}
...
/* Kernel Rmnet driver handle RAW IP packets directly */
SEC("xdp_fp_rawip")
int xdp_fp_prog_rawip(struct xdp_md *ctx)
{
	void *data_end = (void *)(long)ctx->data_end;
	void *data = (void *)(long)ctx->data;
	int *ff_disabled, index_key = GLOB_FF_DISABLE, index_key_stat = GLOB_STAT;
	...
	if (data + 1 > data_end) {
		bpf_debug("%s: Invalid data(%p) + 1 > data_end(%p) => XDP_PASS\n",
			module, data, data_end);
		goto pass;
	}
